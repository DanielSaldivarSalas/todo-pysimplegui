...
class Category:
    ...
    def __init__(self, name: str) -> None:
       ...
       # self.layout.append(sg.Menu(menu_layout))
        ...
        self.layout.append([sg.Listbox(values=self.todo_tasks, size=(60,15), key=f"-{name.upper()}_TODO_TASKS-"),sg.Button("Complete", key=f"{name}_complete"), sg.Button("Delete", key=f"-{name.upper()}_TODO_DELETE-"), sg.Button("Edit")])
        self.layout.append([sg.Text("Completed List")])
        self.layout.append([sg.Listbox(values=self.completed_tasks, size=(60,15), key=f"-{name.upper()}_COMPLETED_TASKS-"),sg.Button("Delete", key=f'-{name.upper()}_COMPLETED_DELETE-')])
